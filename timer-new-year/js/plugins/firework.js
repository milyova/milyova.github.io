window.onload = function () {
    ...
    let menuButtonsOffFirework = [btnBgSpring, btnBgSummer, btnBgAutumn, btnBgWinter, btnBgDecember];
    menuButtonsOffFirework.forEach((elem) => {
        elem.addEventListener('click', function () {
            componentFirework.classList.add('hlp--remove');
        });
    });
    // Запуск фейерверка по кнопке
    btnNewYear.addEventListener('click', function () {
        componentFirework.classList.remove('hlp--remove');
    });
    ...
    function startFireWorks() {
        ...
        setInterval(function () {
            ...
            const leftTime = targetMoment - currentMoment;
            if (leftTime <= 0) {
                componentFirework.classList.remove('hlp--remove');
            }
        }, 1000);
